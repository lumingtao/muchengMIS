...
### 2026-06-18 19:27 CST | 提交新变更并创建内部测试版本
- 分支：main
- 远端：origin/main
...
- 变更内容：
  ...
  - 新建维修工单与工单详情的故障/价格区域继续扩展，支持行级价格编辑、总价调整和更明确的客户/设备录入布局。
  - 后端补充维修工单价格相关输入、仓储和服务逻辑，并同步维修流程测试覆盖。
  - 更新前端构建产物，保证本地启动时加载最新 bundle。
- 同步结果：提交已生成，待打标签并推送远端后确认。
- 验证情况：已运行 `npm.cmd run build`；已运行 `python -m pytest mis_mvp\tests --basetemp .runtime\pytest-tmp`，结果 `53 passed`。
- 回退参考：如需回到本次测试基线，可 checkout `test/v20260618-192700`；如需撤销业务变更，可 revert 本次提交。
- 备注：该标签作为本轮内部测试验收基线，覆盖发券优惠后续价格编辑与实时看板修正。
...
- 提交：469b9ae Document local pytest environment；后续补充同步结果的记录提交以 `git log` 为准
- 变更内容：
...
### 2026-06-18 09:40 CST | 打内部测试版本标签

- 分支：main
- 远端：origin/main
- 操作：基于当前 main 最新提交创建 annotated tag
- 提交：778234f Record sync: merge codex/order-status into main
- 标签：test/v20260618-094006
- 变更内容：无代码变更，仅创建版本标签作为回退检查点。
- 同步结果：标签已推送远端。
- 验证情况：标签指向当前 main HEAD（778234f），本地与远端一致。
- 回退参考：`git checkout test/v20260618-094006` 可回到本次基线；`git revert` 相关提交可撤销业务变更。
- 备注：该标签标记订单状态特殊开发合并后的稳定基线。

### 2026-06-18 21:33 CST | 快进同步远端 main 最新提交

- 分支：main
- 远端：origin/main
- 操作：fetch 远端标签与分支后，将本地 main 快进到 origin/main，并准备提交本次同步留痕
- 提交：
  - 同步前本地基线：d812809 Record test version tag test/v20260618-094006
  - 同步后远端最新：f45839c Record test version tag test/v20260618-192700
  - 本次拉取范围：远端新增 5 个提交，并新增标签 test/v20260618-192700
- 变更内容：
  - 引入维修项目删除、优惠券折扣、内部测试版本与价格相关更新。
  - 更新前端 App.tsx、页面样式与构建产物，补充前端入口同步标记。
  - 更新后端 app/db/models/repository/service 及相关 API 测试和维修流程测试。
  - 新增 Windows 启动脚本 start_project.cmd，更新 start_project.ps1、project_ctl.py 与项目 README。
- 同步结果：本地 main 已从 d812809 快进到 f45839c；当前将通过后续留痕提交再次推送，使远端包含本同步记录。
- 验证情况：本次执行 fetch 与快进合并，无冲突；尚未运行自动化测试或 UI 验证。
- 回退参考：如需回退本次远端同步带来的业务变更，可优先从 f45839c 往前分析本次 5 个远端提交；如只需撤销同步留痕，可 revert 后续记录提交。
- 备注：本次同步标签 test/v20260618-192700 已随 fetch 获取到本地。
