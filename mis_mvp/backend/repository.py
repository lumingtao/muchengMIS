...
class Repository:
    ...
    def delete_machine(self, machine_id: int) -> None:
        ...
        for repair_id in repair_ids:
            self.conn.execute("DELETE FROM repair_order_module_actions WHERE repair_order_id=?", (repair_id,))
            self.conn.execute("DELETE FROM repair_order_modules WHERE repair_order_id=?", (repair_id,))
    ...
    def list_repair_order_modules(self, repair_order_id: int) -> list[dict[str, Any]]:
        rows = self.conn.execute(
            """
            SELECT * FROM repair_order_modules
            WHERE repair_order_id=?
            ORDER BY sequence, module_id
            """,
            (repair_order_id,),
        ).fetchall()
        return [dict(row) for row in rows]

    def get_repair_order_module(self, repair_order_id: int, module_key: str) -> dict[str, Any] | None:
        return row_to_dict(
            self.conn.execute(
                "SELECT * FROM repair_order_modules WHERE repair_order_id=? AND module_key=?",
                (repair_order_id, module_key),
            ).fetchone()
        )

    def ensure_repair_order_module(self, repair_order_id: int, module: dict[str, Any]) -> int:
        existing = self.get_repair_order_module(repair_order_id, str(module["module_key"]))
        if existing:
            return int(existing["module_id"])
        cur = self.conn.execute(
            """
            INSERT INTO repair_order_modules
            (repair_order_id, module_key, module_name, sequence, status, owner_role,
             owner_user_id, blocked_reason, readonly, summary, payload_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                repair_order_id,
                ...
                module["sequence"],
                module["status"],
                module.get("owner_role", ""),
                module.get("owner_user_id", ""),
                module.get("blocked_reason", ""),
                1 if module.get("readonly") else 0,
                module.get("summary", ""),
                module.get("payload_json", "{}"),
            ),
        )
        return int(cur.lastrowid)

    def update_repair_order_module(self, repair_order_id: int, module_key: str, **fields: Any) -> None:
        allowed = {
            "status",
            "owner_role",
            "owner_user_id",
        ...
        values.extend([repair_order_id, module_key])
        self.conn.execute(
            f"UPDATE repair_order_modules SET {', '.join(updates)} WHERE repair_order_id=? AND module_key=?",
            tuple(values),
        )

    def add_repair_order_module_action(
        self,
        repair_order_id: int,
        module_id: int | None,
        module_key: str,
        ...
        cur = self.conn.execute(
            """
            INSERT INTO repair_order_module_actions
            (repair_order_id, module_id, module_key, action_key, action_name, from_status,
             to_status, actor_user_id, actor_name, payload_json, reason, idempotency_key)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                repair_order_id,
    ...
    def list_repair_order_module_actions(self, repair_order_id: int) -> list[dict[str, Any]]:
        rows = self.conn.execute(
            """
            SELECT * FROM repair_order_module_actions
            WHERE repair_order_id=?
