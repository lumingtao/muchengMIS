...
function hasOwnField(row: AnyRecord, key: string) {
  return Object.prototype.hasOwnProperty.call(row, key);
}

function controlledText(row: AnyRecord, key: string, fallback: unknown = "") {
  return hasOwnField(row, key) ? String(row[key] ?? "") : String(fallback ?? "");
}
...
export function splitHonorificName(value: unknown, gender?: unknown) {
  const rawName = String(value ?? "").trim();
  if (rawName.endsWith("女士")) return { name: rawName.slice(0, -2), honorific: "女士" };
  if (rawName.endsWith("先生")) return { name: rawName.slice(0, -2), honorific: "先生" };
  const genderText = String(gender ?? "");
  return {
    name: rawName,
    honorific: genderText === "女" ? "女士" : genderText === "男" ? "先生" : "",
  };
}
...
function TodoOrders({ rows, onOpen }: { rows: AnyRecord[]; onOpen: (row: AnyRecord) => void }) {
  ...
  return (
    <AppTable
      ...
      renderers={{
        order_no: (row) => <AppButton type="link" onClick={() => onOpen(row)}>{String(row.order_no || row.machine_no || "-")}</AppButton>,
        priority: (row) => {
...
function RepairPool({ notify, openOrderDetail, openNewOrder }: { notify: (message: string, error?: boolean) => void; openOrderDetail: (row: AnyRecord, mode?: OrderMode) => void; openNewOrder?: () => void }) {
  ...
  const cancelMutation = useMutation({
    mutationFn: ({ row, reason }: { row: AnyRecord; reason: string }) => api<AnyRecord>(`/api/repair-orders/${row.repair_order_id}/status`, { method: "POST", body: JSON.stringify({ status: "已作废", remark: reason }) }),
    onSuccess: () => {
      notify("订单已取消");
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ row, reason }: { row: AnyRecord; reason: string }) => api<AnyRecord>(`/api/repair-orders/${row.repair_order_id}`, { method: "DELETE", body: JSON.stringify({ reason }) }),
    onSuccess: () => {
      notify("订单已删除并归档");
      queryClient.invalidateQueries({ queryKey: ["repair-workbench"] });
    },
  ...
  const dispatchMutation = useMutation({
    mutationFn: ({ order, engineer }: { order: AnyRecord; engineer: string }) => api<AnyRecord>(`/api/repair-orders/${order.repair_order_id}/assign`, { method: "POST", body: JSON.stringify({ engineer_user_id: engineer, remark: "订单池派单" }) }),
    onSuccess: () => {
      notify("派单已完成");
      setDispatchOrder(null);
      queryClient.invalidateQueries({ queryKey: ["repair-workbench"] });
    },
    onError: error => notify(error instanceof Error ? error.message : "派单失败", true),
  });
  ...
  function cancelOrder(row: AnyRecord) {
    const reason = promptReason("请输入取消原因");
    if (!reason) {
      notify("取消订单必须填写原因", true);
      return;
    }
    cancelMutation.mutate({ row, reason });
  }

  function deleteOrder(row: AnyRecord) {
    const reason = promptReason("请输入删除原因");
    if (!reason) {
      notify("删除订单必须填写原因", true);
      return;
    }
    deleteMutation.mutate({ row, reason });
  }
...
function PoolOrderRow({ row, onOpen, onDispatch, onCancel, onDelete }: { row: AnyRecord; onOpen: (row: AnyRecord, mode?: OrderMode) => void; onDispatch: (row: AnyRecord) => void; onCancel: (row: AnyRecord) => void; onDelete: (row: AnyRecord) => void }) {
  ...
  return (
    <tr>
      ...
      <td className="align-center pool-action-cell"><div className="pool-row-actions">
        ...
        {actions.includes("cancel") && <button type="button" className="danger" onClick={() => onCancel(row)}>取消</button>}
        {actions.includes("delete") && <button type="button" className="danger" onClick={() => onDelete(row)}>删除</button>}
      </div></td>
    </tr>
  );
}
...
function rowActions(row: AnyRecord) {
  return (row.available_actions as string[] | undefined) || ["view"];
}

function orderStatusLight(row: AnyRecord) {
  return String(row.status_light_key || (row.status_light as AnyRecord | undefined)?.key || normalizeRepairStatus(row.status));
}
...
function DispatchOrderModal({
  ...
  loading?: boolean;
  onClose: () => void;
  onSubmit: (engineer: string) => void;
  open: boolean;
  order: AnyRecord | null;
}) {
  const employeesQuery = useQuery({ queryKey: ["dispatch-employees", open], queryFn: () => api<AnyRecord[]>(employeesUrl), enabled: open });
  const employees = ((employeesQuery.data as AnyRecord[] | undefined) || []).filter(row => Boolean(row.username));
  return (
    <AppModal open={open} onClose={onClose} width={920}>
      <header className="modal-header"><div><h2>{order?.assigned_to ? "改派工程师" : "指派工程师"}</h2><p>{String(order?.order_no || "")} · 仅可选择已绑定登录账号且开启接单的工程师</p></div></header>
      <div className="modal-content">
        ...
        <AppTable
          ...
          formatValue={(row, key) => {
            ...
            if (key === "active_order_count") return String(row.active_order_count ?? row.open_order_count ?? 0);
            return displayValue(row, key);
          }}
          isStatusKey={(key) => key === "accepting_orders"}
          actions={{ title: "操作", render: row => <AppButton type="primary" disabled={loading || !row.accepting_orders || !row.username} onClick={() => onSubmit(String(row.username))}>选择</AppButton> }}
          empty="暂无可派单工程师，请先在员工管理绑定登录账号并开启接单"
        />
      </div>
    </AppModal>
  );
...
function CreateRepairOrderModal({ notify, onClose, onCreated, open }: { notify: (message: string, error?: boolean) => void; onClose: () => void; onCreated: () => void; open: boolean }) {
  ...
  const deviceModelsQuery = useQuery({ queryKey: ["device-models", "create-order"], queryFn: () => api<AnyRecord[]>("/api/device-models?enabled_only=true"), enabled: open });
  const repairSkuQuery = useQuery({ queryKey: ["repair-skus", "create-order", model, faultKeyword], queryFn: () => api<AnyRecord[]>(`/api/repair-skus?model=${encodeURIComponent(model)}&q=${encodeURIComponent(faultKeyword)}`), enabled: open && Boolean(model) && faultPickerOpen });
  const customerSearchQuery = useQuery({
    queryKey: ["order-create-modal-customers", customerLookupAnchor, customerRequestKeyword],
    queryFn: () => api<AnyRecord[]>(`/api/customers?q=${encodeURIComponent(customerRequestKeyword)}`),
    enabled: open && customerLookupOpen && hasTextValue(customerLookupKeyword),
  });
  const machineSearchQuery = useQuery({
    queryKey: ["order-create-modal-machines", machineLookupAnchor, machineLookupKeyword],
    queryFn: () => api<AnyRecord[]>(`/api/machines?q=${encodeURIComponent(machineLookupKeyword)}`),
    enabled: open && machineLookupOpen && hasTextValue(machineLookupKeyword),
  });
  ...
  const quoted = selectedFaults.reduce((sum, row) => {
  ...
  function selectCustomer(row: AnyRecord) {
    const nameParts = splitHonorificName(row.name, row.gender);
    const honorific = nameParts.honorific || "先生";
    setSelectedCustomer(row);
    setCustomerHonorific(honorific);
    setCustomerLookupOpen(false);
    setForm(prev => ({
      ...prev,
  ...
  function buildFaultRow(row: AnyRecord) {
    return {
      sku_id: row.sku_id,
      item_name: String(row.fault_name || row.solution_name || "维修故障"),
      fault_name: String(row.fault_name || row.solution_name || "维修故障"),
      sku_code: String(row.sku_code || row.material_code || "-"),
      material_name: row.material_name || "-",
      material_code: row.material_code || "-",
      quantity: 1,
      cost_amount: Number(row.cost_amount || row.material_unit_price || 0),
      charge_amount: Number(row.charge_amount || 0),
      remark: "建单故障选择器生成",
    };
  }

  function addFault(row: AnyRecord) {
    if (!row.sku_id) return;
    setSelectedFaults(prev => prev.some(item => String(item.sku_id) === String(row.sku_id)) ? prev : [...prev, buildFaultRow(row)]);
    setFaultPickerOpen(false);
    setFaultKeyword("");
  }

  function removeFault(index: number) {
  ...
  function createCustomerName() {
    ...
    if (!name) return "";
    return /(?:先生|女士)$/.test(name) ? name : `${name}${customerHonorific}`;
  }
  ...
  return (
    <AppModal open={open} onClose={onClose} width={980}>
      <header className="modal-header"><div><h2>新建维修工单</h2><p>填写设备、客户和故障，确认后直接回到订单池。</p></div></header>
      <div className="modal-content create-order-modal">
        <QueryState loading={deviceModelsQuery.isLoading} error={deviceModelsQuery.error} />
        <div className="order-detail-layout create-order-detail-layout">
          <div className="order-main-column">
            <section className="order-card device-card">
              <div className="repair-card-head"><h3><Smartphone size={24} />设备信息</h3></div>
        ...
        <section className="order-card fault-picker">
          ...
          {faultPickerOpen && (
            <div className="fault-picker-panel">
              <div className="fault-picker-panel-head">
                <b>故障列表</b>
                <Input prefix={<Search size={16} />} value={faultKeyword} onChange={event => setFaultKeyword(event.target.value)} placeholder="搜索故障名称、代码或物料" autoFocus />
                <button type="button" className="table-link" onClick={() => setFaultPickerOpen(false)}>关闭</button>
              </div>
              <QueryState loading={repairSkuQuery.isLoading} error={repairSkuQuery.error} />
              <div className="fault-picker-results">
                {repairSkus.map(row => {
                  const disabled = selectedFaults.some(item => String(item.sku_id) === String(row.sku_id));
                  return (
                    <button type="button" key={String(row.sku_id)} className="fault-picker-option" disabled={disabled} onClick={() => addFault(row)}>
                      <b>{String(row.fault_name || row.solution_name || "维修故障")}</b>
                      <span>{String(row.sku_code || "-")} · {String(row.material_name || "未绑定物料")} · {poolMoney(Number(row.cost_amount || 0) + Number(row.charge_amount || 0))}</span>
                      <em>{disabled ? "已添加" : "选择"}</em>
                    </button>
                  );
                })}
          ...
          <div className="repair-lines fault-picker-selected-table">
          <AppTable
            rows={selectedFaults}
...
function OrderDetailPage({
  ...
  const [deviceForm, setDeviceForm] = useState<AnyRecord>({});
  ...
  const [repairFaultKeyword, setRepairFaultKeyword] = useState("");
  const [selectedCustomer, setSelectedCustomer] = useState<AnyRecord | null>(null);
  const [selectedMachine, setSelectedMachine] = useState<AnyRecord | null>(null);
  ...
  const customerLookupEnabled = mode === "new" || customerEditing;
  const machineLookupEnabled = mode === "new" || deviceEditing;
  const customerLookupSource = customerEditing ? customerForm : form;
  const machineLookupSource = deviceEditing ? deviceForm : form;
  const customerLookupKeyword = customerLookupAnchor === "phone" ? String(customerLookupSource.phone || "") : String(customerLookupSource.customer_name || "");
  const customerRequestKeyword = customerLookupRequestKeyword(customerLookupAnchor, customerLookupKeyword);
  const machineLookupKeyword = machineLookupAnchor === "imei" ? String(machineLookupSource.imei || "") : String(machineLookupSource.serial || "");
  const customerSearchQuery = useQuery({
    queryKey: ["order-create-customers", customerLookupAnchor, customerRequestKeyword],
    queryFn: () => api<AnyRecord[]>(`/api/customers?q=${encodeURIComponent(customerRequestKeyword)}`),
    enabled: customerLookupEnabled && customerLookupOpen && hasTextValue(customerLookupKeyword),
  });
  const machineSearchQuery = useQuery({
    queryKey: ["order-create-machines", machineLookupAnchor, machineLookupKeyword],
    queryFn: () => api<AnyRecord[]>(`/api/machines?q=${encodeURIComponent(machineLookupKeyword)}`),
    enabled: machineLookupEnabled && machineLookupOpen && hasTextValue(machineLookupKeyword),
  });
  const repairSkuQuery = useQuery({
    queryKey: ["repair-skus", form.model || order.model || "", repairFaultKeyword],
    queryFn: () => api<AnyRecord[]>(`/api/repair-skus?model=${encodeURIComponent(String(form.model || order.model || ""))}&q=${encodeURIComponent(repairFaultKeyword)}`),
    enabled: showItemForm && Boolean(form.model || order.model),
  });
  ...
  const deviceModelsQuery = useQuery({
    ...
    enabled: mode === "new" || deviceEditing || customerEditing,
  });
  ...
  const imei = firstText(display.imei, order.imei, order.serial);
  const customer = firstText(display.customer_name, order.customer_name);
  const phone = firstText(display.phone, order.phone, order.customer_phone);
  ...
  const customerNameParts = splitHonorificName(firstText(customerDisplay.customer_name), customerDisplay.gender || order.gender);
  ...
  const detailQuoted = detailRows.reduce((sum, row) => {
    const { lineAmount } = repairLineAmounts(row);
    return sum + lineAmount * firstNumber(row.quantity, row.qty, 1);
  }, 0);
  ...
  const repairSkuRows = ((repairSkuQuery.data as AnyRecord[] | undefined) || []);
  const canUseFaultPicker = mode === "new" || !isOrderReadonly;
  ...
  const deviceMutation = useMutation({
    mutationFn: (payload: AnyRecord) => {
      ...
      if (!order.repair_order_id && !orderId) throw new Error("缺少维修单 ID，无法编辑设备信息");
      return api<AnyRecord>(`/api/repair-orders/${order.repair_order_id || orderId}/machine`, {
        method: "PUT",
        body: JSON.stringify({
          imei: payload.imei || "",
  ...
  const customerMutation = useMutation({
    mutationFn: (payload: AnyRecord) => {
      if (!order.machine_id) throw new Error("缺少机器档案，无法编辑客户信息");
      const honorific = String(payload.honorific || "先生");
      const nameParts = splitHonorificName(payload.customer_name, honorific === "女士" ? "女" : "男");
      const nameWithHonorific = nameParts.name.trim();
      const fullName = nameWithHonorific ? nameWithHonorific + honorific : "";
      const nextPhone = Object.prototype.hasOwnProperty.call(payload, "phone") ? payload.phone : order.customer_phone;
      const nextCustomerType = Object.prototype.hasOwnProperty.call(payload, "customer_type") ? payload.customer_type : order.customer_type;
      return api<AnyRecord>(`/api/machines/${order.machine_id}`, {
        ...
        body: JSON.stringify({
          ...
          customer: {
            name: fullName || order.customer_name || "待补",
            phone: nextPhone ?? "",
            gender: honorific === "女士" ? "女" : "男",
            category: nextCustomerType || "个人客户",
          },
        }),
      });
    },
  ...
  const completeOrderMutation = useMutation({
    mutationFn: () => api<AnyRecord>(`/api/repair-orders/${order.repair_order_id || orderId}/status`, { method: "POST", body: JSON.stringify({ status: "已结单", remark: "临时手动完结" }) }),
    onSuccess: () => {
      notify("订单已临时完结");
      setForm({});
      onModeChange("view");
  ...
  function submitDevice() {
    if (!String(deviceForm.model || "").trim()) {
      notify("请填写手机型号", true);
      return;
    }
    deviceMutation.mutate(deviceForm);
  }
  function beginCustomerEdit() {
    if (!order.machine_id && mode !== "new") {
      notify("缺少机器档案，无法编辑客户信息", true);
      return;
    }
    const nameParts = splitHonorificName(order.customer_name, order.gender);
    const honorific = nameParts.honorific || "先生";
    setCustomerHonorific(honorific);
    setCustomerForm({
  ...
  function setCustomerField(key: string, value: unknown) {
  ...
  function submitCustomer() {
    if (!String(customerForm.customer_name || "").trim()) {
      notify("请填写客户姓名", true);
      return;
    }
    customerMutation.mutate({ ...customerForm, honorific: customerHonorific });
  }
  ...
  function saveInspection(stage: "pre" | "post") {
    if (isOrderReadonly) {
      notify("当前订单为只读状态，不能修改检测结果", true);
      return;
    }
  ...
  function selectCustomer(row: AnyRecord) {
    ...
    if (customerEditing) {
      setCustomerHonorific(honorific);
      setCustomerForm(prev => ({ ...prev, ...nextCustomer, honorific }));
      return;
    }
    setForm(prev => ({
      ...prev,
  ...
  function selectMachine(row: AnyRecord) {
    ...
    if (deviceEditing) {
      setDeviceForm(prev => ({ ...prev, ...nextMachine }));
      return;
    }
    setForm(prev => ({
      ...prev,
  ...
  function buildDetailFaultRow(row: AnyRecord) {
    return {
      ...
      cost_amount: Number(row.cost_amount || row.material_unit_price || 0),
      charge_amount: Number(row.charge_amount || 0),
      remark: row.remark || "故障选择器生成",
    };
  }
  function addDetailFault(row: AnyRecord) {
    const next = buildDetailFaultRow(row);
    if (mode === "new") {
      setNewRepairItems(prev => prev.some(item => next.sku_id && String(item.sku_id) === String(next.sku_id)) ? prev : [...prev, next]);
      setShowItemForm(false);
      setRepairFaultKeyword("");
      return;
    }
        ...
        <div className="order-main-column">
          <section className="order-card device-card">
            <div className="repair-card-head">
              ...
              <div className="inline-actions">
                {mode !== "new" && !isOrderReadonly && (deviceEditing ? (
                  <>
                    <button type="button" className="mini-add-button" onClick={submitDevice} disabled={deviceMutation.isPending}>保存</button>
                    <button type="button" className="ghost-mini-button" onClick={() => { setDeviceEditing(false); setDeviceForm({}); }}>取消</button>
                  </>
                ) : (
                  <button type="button" className="ghost-mini-button" onClick={beginDeviceEdit} disabled={!order.machine_id}><Edit3 size={15} />编辑</button>
                ))}
              </div>
            </div>
            <div className="order-info-grid">
          ...
          <section className="order-card customer-card">
            <div className="repair-card-head">
              ...
              <div className="inline-actions">
                {mode !== "new" && !isOrderReadonly && (customerEditing ? (
                  <>
                    <button type="button" className="mini-add-button" onClick={submitCustomer} disabled={customerMutation.isPending}>保存</button>
                    <button type="button" className="ghost-mini-button" onClick={() => { setCustomerEditing(false); setCustomerForm({}); }}>取消</button>
                  </>
                ) : (
                  <button type="button" className="ghost-mini-button" onClick={beginCustomerEdit}><Edit3 size={15} />编辑</button>
                ))}
              </div></div>
            <div className="order-info-grid compact">
              <OrderEditableLine label="客户姓名" value={customerName} editable={mode === "new" || customerEditing} tag={!customerEditing && customerName && customerHonorificTag ? customerHonorificTag : ""} onFocus={() => { if (customerLookupEnabled) updateCustomerLookup("name", customerLookupSource.customer_name); }} onBlur={() => { if (customerLookupEnabled) window.setTimeout(() => setCustomerLookupOpen(false), 120); }} onKeyDown={event => { if (customerLookupEnabled && ["Enter", "Escape"].includes(event.key)) setCustomerLookupOpen(false); }} onChange={v => { customerEditing ? setCustomerField("customer_name", v) : setField("customer_name", v); }} />
              {mode === "new" || customerEditing ? (
                <div className="info-line order-editable-line order-honorific-group">
                  <span className="order-field-label">称呼</span>
                  <Radio.Group value={customerHonorific} onChange={e => setCustomerHonorific(e.target.value)} size="small">
                    <Radio.Button value="先生">先生</Radio.Button>
                    <Radio.Button value="女士">女士</Radio.Button>
                  </Radio.Group>
                </div>
              ) : (
                <div className="info-line order-editable-line order-layout-spacer" aria-hidden="true" />
              )}
              <OrderEditableLine label="联系方式" value={customerPhone} editable={mode === "new" || customerEditing} highlight onFocus={() => { if (customerLookupEnabled) updateCustomerLookup("phone", customerLookupSource.phone); }} onBlur={() => { if (customerLookupEnabled) window.setTimeout(() => setCustomerLookupOpen(false), 120); }} onKeyDown={event => { if (customerLookupEnabled && ["Enter", "Escape"].includes(event.key)) setCustomerLookupOpen(false); }} onChange={v => { customerEditing ? setCustomerField("phone", v) : setField("phone", v); }} />
            {customerLookupEnabled && customerLookupOpen && (customerSearchQuery.isLoading || customerOptions.length > 0) && <SuggestionList className={`customer-suggestions lookup-anchor-left ${customerLookupAnchor === "phone" ? "lookup-row-2" : "lookup-row-1"}`} loading={customerSearchQuery.isLoading} rows={customerOptions} selectedId={selectedCustomer?.customer_id} idKey="customer_id" primaryKey="name" secondaryKeys={["member_no", "phone", "vip_level", "tags", "category", "shop_name"]} onSelect={selectCustomer} empty="没有找到已有客户" />}
              <OrderChoiceLine label="客户标签" value={customerTypeDisplay} editable={mode === "new" || customerEditing} options={["个人客户", "同行客户"]} onChange={v => customerEditing ? setCustomerField("customer_type", v) : setField("customer_type", v)} />
              {mode === "new" && <OrderField label="客户备注" value={display.customer_remark} editable area onChange={v => setField("customer_remark", v)} />}
            </div>
          </section>
          <EditableInspectionCard title="维修前检测 (Pre-Repair)" inspections={inspectionItems} note={preInspectionNote} mode={mode} state={preInspectionState} photos={prePhotos} stage="pre" editing={preInspectionEditing} saved={mode === "new" && preInspectionSaved} readonly={isOrderReadonly} onStart={() => { setPreInspectionSaved(false); setPreInspectionEditing(true); }} onSave={() => saveInspection("pre")} onCancel={() => { setPreInspectionSaved(true); setPreInspectionEditing(false); }} onToggle={item => toggleInspection("pre", item)} onNoteChange={setPreInspectionNote} onUpload={uploadPhoto} onOpenPhoto={setSelectedPhoto} uploading={photoMutation.isPending || inspectionMutation.isPending} />
          ...
          <section className="order-card">
            ...
            {showItemForm && canUseFaultPicker && (
              <div className="fault-picker-panel">
                <div className="fault-picker-panel-head">
                  <b>故障列表</b>
                  <Input prefix={<Search size={16} />} value={repairFaultKeyword} onChange={event => setRepairFaultKeyword(event.target.value)} placeholder="搜索故障名称、代码或物料" autoFocus />
                  <button type="button" className="table-link" onClick={() => { setShowItemForm(false); setRepairFaultKeyword(""); }}>关闭</button>
                </div>
                <QueryState loading={repairSkuQuery.isLoading} error={repairSkuQuery.error} />
                <div className="fault-picker-results">
                  {repairSkuRows.map(row => {
                    const disabled = detailRows.some(item => row.sku_id && String(item.sku_id || "") === String(row.sku_id));
                    return (
                      <button type="button" key={String(row.sku_id)} className="fault-picker-option" disabled={disabled || addItemMutation.isPending} onClick={() => addDetailFault(row)}>
                        <b>{String(row.fault_name || row.solution_name || "维修故障")}</b>
                        <span>{String(row.sku_code || "-")} · {String(row.material_name || "未绑定物料")} · {poolMoney(Number(row.cost_amount || 0) + Number(row.charge_amount || 0))}</span>
                        <em>{disabled ? "已添加" : "选择"}</em>
                      </button>
...
<div className="repair-lines fault-picker-selected-table">
  ...
  />
        ...
        <aside className="order-side-column">
          ...
          {isReadOnlyArchive && <section className="order-card archive-info-card"><h3>删除归档信息</h3><p>该订单已从普通订单池隐藏，仅可通过完整订单编号搜索查看。</p><div><b>删除原因</b><span>{String(archiveMeta.archive_reason || order.archive_reason || "-")}</span></div><div><b>操作人</b><span>{String(archiveMeta.archived_by || order.archived_by || "-")}</span></div><div><b>彻底删除时间</b><span>{String(archiveMeta.purge_after || order.purge_after || "-")}</span></div></section>}
...
<section className="order-card notes-card">
  <div className="side-title-row"><h3>备注信息</h3>{!isOrderReadonly && <button type="button" className="icon-mini-button" onClick={() => { setNoteFormOpen(value => !value); setNoteForm({ type: "内部备注" }); }}><CirclePlus size={22} /></button>}</div>
  {noteFormOpen && (
  ...
  {visibleOrderNotes.map((row, index) => { const noteType = String(row.note_type || row.type || "内部备注"); const createdBy = String(row.created_by || row.operator || currentOperator); const createdAt = String(row.created_at || "待创建"); return <div className={'note-box note-entry ' + (noteType === "交付说明" ? "muted" : "warning")} key={String(row.note_id || row.temp_id || noteType + '-' + index)}><div className="note-entry-head"><b>{noteType}</b><span>{createdBy} ? {createdAt}</span></div><p>{String(row.content || "")}</p>{!isOrderReadonly && !row.readonly && <div className="note-entry-actions"><AppButton type="link" onClick={() => editNote(row, index)}>修改</AppButton><AppButton type="link" danger onClick={() => deleteNote(row, index)} disabled={deleteNoteMutation.isPending}>删除</AppButton></div>}</div>; })}
</section>
...
function canModifyRepairItems(status: string) {
  return !["已完结", "已取消", "已作废", "已删除"].includes(status);
}
...
function LegacyOrderDetailPage({ orderId, notify, onBack }: { orderId: number | string | null; notify: (message: string, error?: boolean) => void; onBack: () => void }) {
  ...
  const model = firstText(order.model);
  const colorCapacity = [firstText(order.color), firstText(order.memory, order.capacity)].filter(Boolean).join(" / ");
  const imei = firstText(order.imei, order.serial);
  const customer = splitHonorificName(order.customer_name, order.gender).name;
  const phone = firstText(order.phone, order.customer_phone);
...
function EditableInspectionCard({ title, inspections, note, compact, mode = "view", state = {}, photos = [], stage, editing = false, saved = false, readonly = false, onStart, onSave, onCancel, onToggle, onNoteChange, onUpload, onOpenPhoto, uploading }: { title: string; inspections: string[]; note: string; compact?: boolean; mode?: OrderMode; state?: Record<string, boolean>; photos?: AnyRecord[]; stage?: "pre" | "post"; editing?: boolean; saved?: boolean; readonly?: boolean; onStart?: () => void; onSave?: () => void; onCancel?: () => void; onToggle?: (item: string) => void; onNoteChange?: (value: string) => void; onUpload?: (stage: "pre" | "post", file: File | null) => void; onOpenPhoto?: (photo: AnyRecord) => void; uploading?: boolean }) {
  const editable = readonly || saved ? false : mode === "new" || mode === "edit" || editing;
    ...
    <section className={'order-card inspection-card ' + (compact ? 'compact' : '')}>
      <div className="inspection-head">
        ...
        <div className="inspection-actions">
          {!readonly && !editable && <AppButton className="ghost-mini-button" onClick={onStart}>{saved ? "修改检测结果" : "变更检测结果"}</AppButton>}
      ...
      <label className="inspection-note-label">故障描述<Input.TextArea className={'inspection-note ' + (needsOtherNote && !String(note || "").trim() ? 'required' : '') + (editable ? '' : ' readonly')} value={note} disabled={!editable} placeholder={needsOtherNote ? "请填写其他异常说明..." : "检测备注..."} onChange={event => onNoteChange?.(event.currentTarget.value)} /></label>
      {!compact && <div className="photo-strip">{photos.length ? photos.map(photo => <button type="button" className="photo-thumb-button" key={String(photo.photo_id || photo.url)} onClick={() => onOpenPhoto?.(photo)}><img className="photo-thumb" src={String(photo.url || "")} alt={String(photo.filename || "维修照片")} /></button>) : <div className="photo-thumb" />}{stage && !readonly && <label className={'upload-tile ' + (uploading ? 'disabled' : '')}><Camera size={24} />{uploading ? "上传中" : "上传"}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={event => { onUpload?.(stage, event.currentTarget.files?.[0] || null); event.currentTarget.value = ""; }} /></label>}</div>}
    </section>
  );
}
...
function EmployeeSettings({ notify }: { notify: (message: string, error?: boolean) => void }) {
  ...
  const acceptingCount = allRows.filter(row => row.accepting_orders).length;
  const closedCount = Math.max(allRows.length - acceptingCount, 0);
  const engineerCount = allRows.filter(row => String(row.position || "") === "工程师").length;
  ...
  function setEmployeeField(key: string, value: unknown) {
    setEmployeeForm(prev => ({ ...prev, [key]: value }));
  }

  function toggleEmployeeSkill(tag: string) {
    setEmployeeForm(prev => {
      const current = optionArray(prev.skill_tags);
      const next = current.includes(tag) ? current.filter(item => item !== tag) : [...current, tag];
      return { ...prev, skill_tags: next };
    });
  }

  function submitEmployeeForm() {
    if (!hasTextValue(employeeForm.name)) {
      notify("请填写姓名", true);
      return;
    }
  ...
  return (
    <div className="device-model-page employee-page">
      <section className="device-model-control">
        <div className="device-model-search">
          <Input prefix={<Search size={16} />} value={keyword} onChange={event => setKeyword(event.target.value)} placeholder="搜索姓名、职位、能力标签" allowClear />
          <Select allowClear value={departmentFilter || undefined} onChange={value => setDepartmentFilter(value || "")} placeholder="全部部门" options={departmentOptions} />
          <Select allowClear value={statusFilter || undefined} onChange={value => setStatusFilter(value || "")} placeholder="全部接单状态" options={[{ value: "true", label: "开启接单" }, { value: "false", label: "关闭接单" }]} />
        </div>
        <div className="device-model-actions">
          <AppButton type="primary" onClick={openNewEmployeeForm}><UserPlus size={16} />新增员工</AppButton>
        </div>
      </section>

      <section className="device-model-stats">
        <div><span>员工总数</span><strong>{allRows.length}</strong></div>
        <div><span>工程师</span><strong>{engineerCount}</strong></div>
        <div><span>开启接单</span><strong>{acceptingCount}</strong></div>
        <div><span>关闭接单</span><strong>{closedCount}</strong></div>
      </section>
      ...
      <section className="device-model-table-card">
        <div className="device-model-table-title">
          <div>
            <h2>员工管理</h2>
            <p>维护工程师职位、挂单数量、所属部门、能力标签和接单状态。能力标签可用逗号、顿号或换行分隔。</p>
          </div>
          <span>{allRows.length} 条</span>
        </div>
        <QueryState loading={query.isLoading} error={query.error} />
        <AppTable
          rows={allRows}
          columns={[["name", "姓名"], ["username", "登录账号"], ["position", "职位"], ["open_order_count", "挂单数量"], ["department", "所属部门"], ["skill_tags", "能力标签"], ["accepting_orders", "接单状态"]]}
          formatValue={(row, key) => {
            if (key === "skill_tags") return optionArray(row.skill_tags).join("、") || "-";
            if (key === "accepting_orders") return row.accepting_orders ? "开启" : "关闭";
            return displayValue(row, key);
      ...
      <AppModal open={formOpen} onClose={() => setFormOpen(false)} width={860}>
        <form className="app-form-section employee-form" onSubmit={event => { event.preventDefault(); submitEmployeeForm(); }}>
          <div className="app-form-section-header"><h2>员工档案</h2></div>
          <div className="app-form-section-grid">
            <label>
              <span>姓名</span>
              <Input value={controlledText(employeeForm, "name")} onChange={event => setEmployeeField("name", event.target.value)} placeholder="工程师姓名" />
            </label>
            <label>
              <span>绑定登录账号</span>
            ...
            <label>
              <span>所属部门</span>
              <Input value={controlledText(employeeForm, "department", "维修部")} onChange={event => setEmployeeField("department", event.target.value)} placeholder="维修部" />
            </label>
            <label>
              <span>挂单数量</span>
              <Input min={0} value={Number(employeeForm.open_order_count || 0)} onChange={event => setEmployeeField("open_order_count", event.target.value)} type="number" />
            </label>
            <label>
              <span>接单状态</span>
              <Select value={String(employeeForm.accepting_orders ?? "true")} onChange={value => setEmployeeField("accepting_orders", value)} options={[{ value: "true", label: "开启" }, { value: "false", label: "关闭" }]} />
            </label>
            <div className="employee-skill-field wide">
              <span>能力标签</span>
              <div className="employee-skill-toggle-group">
                {employeeSkillTags.map(tag => {
                  const active = optionArray(employeeForm.skill_tags).includes(tag);
                  return (
                    <button type="button" key={tag} className={active ? "active" : ""} onClick={() => toggleEmployeeSkill(tag)}>
                      {tag}
                    </button>
                  );
                })}
            ...
            <label className="wide">
              <span>备注</span>
              <Input.TextArea value={controlledText(employeeForm, "remark")} onChange={event => setEmployeeField("remark", event.target.value)} placeholder="备注" rows={3} />
            </label>
          </div>
          <div className="app-form-section-actions">
            <AppButton htmlType="submit" loading={mutation.isPending} type="primary"><Plus size={16} />保存员工</AppButton>
            <AppButton onClick={() => setEmployeeForm(emptyEmployeeForm)}>清空</AppButton>
          </div>
        </form>
      </AppModal>
    </div>
...
function DeviceModelSettings({ notify }: { notify: (message: string, error?: boolean) => void }) {
  ...
  const modelFields = [
    ...
    { name: "model_numbers_text", label: "设备编码", placeholder: "A1822、A1823、iPad7,5", area: true },
  ...
  return (
    <div className="device-model-page">
      ...
      <AppTable
        rows={rows}
        columns={[["brand", "品牌"], ["model_name", "型号"], ["sort_order", "排序"], ["enabled", "状态"], ["model_numbers", "设备编码"], ["colors", "颜色"], ["capacities", "容量/内存"]]}
        formatValue={(row, key) => {
