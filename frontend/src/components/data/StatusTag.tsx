...
const doneStatuses = new Set(["已交付", "已完结", "已结单", "成功", "财务已确认", "已售出", "已完成"]);
const pendingStatuses = new Set(["维修中", "待检测", "待报价确认", "待交付检测", "检测中", "已报价", "待分配", "待备料"]);
const warningStatuses = new Set(["同行挂账", "财务待确认", "未收款", "待支付"]);
const successStatuses = new Set(["回收库存", "在库可用", "启用"]);
const errorStatuses = new Set(["已取消", "取消", "失败", "异常"]);
